...
We compare two models: a graph convolutional network baseline following \cite{cranmer2020discoveringsymbolicmodelsdeep}, and a GKAN that replaces selected MLP components with KAN layers. Both models are trained on the same data, with the same loss and optimization setup, to keep the comparison fair.
...
\hs{state what we expect to learn}
...
\ek{I don't know if we should mention sparse init}
...
\section{Discussion}
\subsection{Regularization}
It was expected that the L1-norm and entropy regularizations alone would be insufficient, though even combined---as outlined in \cite{liu2024kankolmogorovarnoldnetworks} to be a better approach---results are disappointing. The differences between losses in the experiments is easily explained as noise, and none of the regularization techniques encouraged sparse networks. This may be explained by:
\begin{enumerate}
    \item The gravitational $n$-body problem requiring dense networks to accurately represent the dynamic system. Though  \cite{liu2024kan20kolmogorovarnoldnetworks} provide an example of a symbolic KAN modeling gravitational force between two bodies in a small, interpretable network, indicating that the gravitational interactions do not require dense networks in the ideal case. \ek{I HATE THIS I HATE THIS THIS}
...
\paragraph{Optimization difficulty of spline-based edge functions.}
Each KAN edge learns a nonlinear spline, so the model must learn both the function value and its local gradient behavior. Flat spline regions can suppress gradients, while steep regions can create unstable updates. This becomes more difficult when many KAN layers are composed.

For gravity, the learned interaction also involves inverse-distance terms, which are asymptotic and poorly conditioned for spline approximation. This likely made optimization especially difficult. In support of this, enforcing a minimum separation distance between bodies noticeably improved training, whereas unconstrained runs reduced loss by less than $5\%$. Moreover, replacing gravity with the better-conditioned Hooke's law produced much larger loss decreases of roughly $90\%$.

\paragraph{Interaction with message passing.}
Message passing may further amplify these issues, since errors in learned edge functions propagate across the graph. Prior work has identified oversmoothing, oversquashing, and gradient degradation in deeper GNNs \parencite{arroyo2025vanishinggradientsoversmoothingoversquashing}. Our results are consistent with this behavior. We tested Adam, L-BFGS, warmup schedules, and alternating optimization, but none consistently improved training. This suggests that optimization may be intrinsically challenging for GKANs in message-passing settings.

\ag{discuss potential for physically informed loss function (e.g., incorporating conservation laws or physical constraints into the loss function, hamiltonian loss functions, etc). This may partially explain poor learning results over all}
...
\hs{adaptive grid updates i guess? dont think this would matter that much tbh}
...
Based on KA representation theory, there exists a model with the fewest possible parameters \hs{elaborate on said KA representation theory with more math} that can still represent the function we want to learn. The idea is to try several of these small models to see if it can learn without overparameterizing. We can also try augmenting the dataset with computed features; this makes the learned mathematical relationships more simple, which keeps the model small even though we introduced new features. In some cases, it reduces the size of the model even further.

\hs{also show architecture layout of said minimal models. Justification can be brief, longer stuff in appendix.}

Here is a summary of the augmented features we tried.

\begin{center}

\begin{tabular}{l|l}
\texttt{rel\_pos} & $x_{j} - x_{i}$ \\
\texttt{dist\_sq} & $\|x_{j} - x_{i}\|^{2}$ \\
\texttt{inv\_dist\_sq} & $(\|x_{j} - x_{i}\|^{2} + \epsilon^{2})^{-1}$
% excluding mass_weighted_rel_pos because in my opinion it doesn't really make sense and it didn't really improve the model. can discuss it if you think it's still relevant
\end{tabular}

\end{center}

We tried 5 small models with a few combinations of augmented features. Here is a summary of the training and validation loss. All models were trained for 50 epochs on the same dataset, which consists of 200 training trajectories and 20 validation trajectories. Refer to my \texttt{experiments.yaml} for exact details, including the rationale for the layer widths and depth of each model.
% git checkout arthur/feat

\begin{center}

\begin{tabular}{l|c|c|c}
experiment & parameters & training loss & validation loss \\
\hline
\texttt{baseline} & $427002$ & $1.783 \times 10^{-2}$ & $1.289 \times 10^{-2}$ \\
\texttt{minimal} & $1964$ & $1.418 \times 10^{-1}$ & $1.360 \times 10^{-1}$ \\
\texttt{minimal\_merged} & $2220$ & $1.130 \times 10^{-1}$ & $1.185 \times 10^{-1}$ \\
\texttt{minimal\_rel\_pos} & $2020$ & $1.873 \times 10^{-2}$ & $1.639 \times 10^{-2}$ \\
\texttt{minimal\_rel\_pos\_dsq} & $2112$ & $1.960 \times 10^{-2}$ & $1.172 \times 10^{-2}$ \\
\texttt{minimal\_rel\_pos\_idc} & $1824$ & $2.059 \times 10^{-2}$ & $1.558 \times 10^{-2}$ \\
% bad naming here due to my mistake, idc = inv_dist_sq. and although the experiments are named "minimal" it's not necessarily the case that these are the smallest possible models, they're just reasonably small.
\end{tabular}

\end{center}

The small models without any augmentation perform about 1 order of magnitude worse compared to the baseline. This alone is still quite impressive considering the difference in the total number of parameters. However, just by augmenting with \texttt{rel\_pos}, we start to see comparable results without significantly increasing the model complexity. The GKAN could be a highly efficient architecture for this use case.

...
% \cite{arroyo2025vanishinggradientsoversmoothingoversquashing}
...
Message passing may further amplify these issues, since errors in learned edge functions propagate across the graph. Prior work has identified oversmoothing, oversquashing, and gradient degradation in deeper GNNs \cite{arroyo2025vanishinggradientsoversmoothingoversquashing}. Our results are consistent with this behavior. We tested Adam, L-BFGS, warmup schedules, and alternating optimization, but none consistently improved training. This suggests that optimization may be intrinsically challenging for GKANs in message-passing settings.



\section{Conclusion}

Although raw performance was weaker than expected, several directions may improve learning:

\begin{itemize}
    \item \textbf{Physics-informed objectives:} include conservation of energy, momentum, or Hamiltonian consistency in the loss.
    \item \textbf{Adaptive KAN parameterization:} use grid updates, better knot placement, or multiplicative nodes.
    \item \textbf{Two-stage training:} first optimize predictive accuracy, then sparsify or simplify the learned functions.
\end{itemize}
